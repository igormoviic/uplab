...
if ($SendCadImg) {
    ...
    //var_dump($_FILES['img']);
    //Inserir no BD
    $result_img = "INSERT INTO banner ( nome, img) VALUES (:nome, :img)";
    $insert_msg = $conn->prepare($result_img);

    $insert_msg->bindParam(':nome', $nome);
    $insert_msg->bindParam(':img', $img);
    ...
    if ($insert_msg->execute()) {
        ...
        if(move_uploaded_file($_FILES['img']['tmp_name'], $diretorio.$img)){
            $_SESSION['msg'] = "<p style='color:green;'>Dados salvo com sucesso e upload da img realizado com sucesso</p>";
